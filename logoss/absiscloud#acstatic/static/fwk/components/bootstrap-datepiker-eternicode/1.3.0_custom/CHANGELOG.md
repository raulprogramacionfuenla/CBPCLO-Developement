...
* Fixed a display bug with component input-group-addon icons being vertically higher than they should have been.
...
* Component pickers are now aligned under the input, not the input-group-addon element.
...
* Component triggers now include `.btn` in addition to `.input-group-addon`.
