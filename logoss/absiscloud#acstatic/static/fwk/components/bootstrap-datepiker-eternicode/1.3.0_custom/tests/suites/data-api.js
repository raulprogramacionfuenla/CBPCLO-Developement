...
test('DATA-API: data-provide="datepicker" on component', function(){
    ...
    html = '<div class="input-group date" data-provide="datepicker">'+
                '<input><span class="input-group-addon"><i class="icon-th"></i></span>'+
            '</div>';
    ...
    comp = $(html).appendTo('#qunit-fixture');
    comp.find('.input-group-addon').focus();
    ok(comp.data('datepicker'), 'append component initialized by "focus" event on input-group-addon');
    comp.remove();

    comp = $(html).appendTo('#qunit-fixture');
    comp.find('.input-group-addon').click();
    ok(comp.data('datepicker'), 'append component initialized by "click" event on input-group-addon');
    comp.remove();


    html = '<div class="input-prepend date" data-provide="datepicker">'+
                '<span class="input-group-addon"><i class="icon-th"></i></span><input>'+
            '</div>';
    ...
    comp = $(html).prependTo('#qunit-fixture');
    comp.find('.input-group-addon').focus();
    ok(comp.data('datepicker'), 'prepend component initialized by "focus" event on input-group-addon');
    comp.remove();

    comp = $(html).prependTo('#qunit-fixture');
    comp.find('.input-group-addon').click();
    ok(comp.data('datepicker'), 'prepend component initialized by "click" event on input-group-addon');
    comp.remove();
});
