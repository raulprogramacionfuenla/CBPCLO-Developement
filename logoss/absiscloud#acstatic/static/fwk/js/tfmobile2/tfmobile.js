...
$(document).bind("mobileinit", function(){
	...
	/*Inicializamos el mapping de jquery*/
	inicializeTFMobileThemes();


});

$(document).on( "pagebeforecreate", function( event ) {
  	renderTFMobileThemes();
});
...
function initProcessToolbar() {
	$(".process-toolbar").each(function() {
		...
		$this.find(".numeral-back").width($this.find(".numeral").width() + parseInt($this.find(".numeral").css('margin-left')) + parseInt($this.find(".numeral").css('margin-right')));
	});
}
...
function initFieldFormaters(){
	...
	$('a[data-role="telf"]').each(function(){
		...
		$this.html(telephone.replace(/(\d{3})(\d{3})(\d{3})/, '$1 $2 $3'));
...
function initList() {
	...
	$('li.list-expanded[data-link]').each(function(){
		$(this).append('<a href="'+$(this).attr("data-link")+'" class="ui-li-link-alt ui-btn link-detail" ><span class="ui-btn-inner"><img class="fletxa-list-expanded" src="../../static/images/tfmobile2/llistes-arrow.png"/></span></a>');
	});
}
...
function initButtonDateBox() {
	...
	$('.datebox').each(function(){
		...
		$this.bind('click', function() {
			...
			$('div.ui-datebox-container .ui-header .ui-btn').text("").removeClass("ui-icon-delete").addClass("ui-icon-cancelar");
			initDateBoxButtons();
		});		
	});
...
function initDateBoxButtons() {
	$('.ui-datebox-gridheader .ui-datebox-gridminus a, .ui-datebox-gridheader .ui-datebox-gridplus a').text("");
	$('.ui-datebox-gridheader .ui-datebox-gridminus').removeClass("ui-icon-arrow-l").addClass("ui-icon-carat-l");
	$('.ui-datebox-gridheader .ui-datebox-gridplus').removeClass("ui-icon-arrow-r").addClass("ui-icon-carat-r");
	
	$('.ui-datebox-gridheader .ui-datebox-gridminus, .ui-datebox-gridheader .ui-datebox-gridplus').bind('click', initDateBoxButtons);
	$('div.ui-datebox-container').bind('swiperight swipeleft', initDateBoxButtons);
}
...
function initListaAutomatica(idPagina, idLista, tamPagina, elementosRecuperados, url){//}, callback) {	
	if ($("#" + idPagina).length>0) {
		...
		$( "#" + idPagina ).lazyloader(); 	
		$.mobile.lazyloader.prototype._instances=new Array();
...
function initTooltipToolbar() {
	...
	$('.ui-header h1.ui-title').bind('tap',function(event) {
	    ...
	    $this.toggleClass('header-toogled-tooltip');
	});

	$('.ui-content').bind('tap',function(event) {
		$('.ui-header h1.ui-title').parent().find(".tooltip").hide();
		$('.ui-header h1.ui-title').removeClass('header-toogled-tooltip');
	})
}
...
/**
 * Esta función hace que el botón de filtrar del header cambie de estado a encendido (azul claro) o apagado (azul oscuro)
 */

function toggleFilterButton (on) {
	if (on) {
		$('.ui-header .ui-icon-filter').addClass('ui-icon-filter-toggled');
	} else {
		$('.ui-header .ui-icon-filter').removeClass('ui-icon-filter-toggled');
	}
}
/**
 * Esta función hace que el botón de biscar del header cambie de estado a encendido (azul claro) o apagado (azul oscuro)
 */

function toggleSearchButton (on) {
	if (on) {
		$('.ui-header .ui-icon-search').addClass('ui-icon-search-toggled');
	} else {
		$('.ui-header .ui-icon-search').removeClass('ui-icon-search-toggled');
	}
}
/**
 * Esta función inicializa los inputs que tienen botón de borrado
 */
function initInputClearButton() {
	$('.ui-input-has-clear input').bind('click', function(e) {
		$('.ui-input-has-clear a').text("");
	});	
}

/**
*Esta función cambia la toolbar de TF Mobile:Componentes a Seleccionar todos
*/
function initCheckElementList(){
	var numberOfChecked = $('input:checkbox:checked').length;
	var totalCheckboxes = $('input:checkbox').length;
	var numberNotChecked = totalCheckboxes - numberOfChecked;
	if (numberOfChecked!=0){
		$('div[data-role="header"]').addClass('hidden');
		$('#toolbar-check').removeClass('hidden');
	}else{
		$('div[data-role="header"]').removeClass('hidden');
		$('#toolbar-check').addClass('hidden');		
	}
	document.getElementsByClassName("contador-selector")[0].textContent='('+(numberOfChecked)+')' ;
}

/**
*Esta función hace que el botón Seleccionar todos y Deseleccionar todos funcione correctamente. Mirar también pulsarcheck()
*/
function initToolbarCheckElementList(){
	var numberOfChecked = $('input:checkbox:checked').length;
	var totalCheckboxes = $('input:checkbox').length;
	var title = document.getElementsByClassName("ui-title")[1].textContent; 
	//console.log(title);
	if (numberOfChecked!=totalCheckboxes){
		$('.ui-toolbar-checkbox label').removeClass('ui-checkbox-off');
		$('.ui-toolbar-checkbox label').addClass('ui-checkbox-on');
		$('.ui-toolbar-checkbox input:checkbox').data('data-cacheval', "false");
		$('.ui-toolbar-checkbox input:checkbox').each(function() {this.checked = true;});

	}else{
		$('.ui-toolbar-checkbox label').removeClass('ui-checkbox-on');
		$('.ui-toolbar-checkbox label').addClass('ui-checkbox-off');
		$('.ui-toolbar-checkbox input:checkbox').data('data-cacheval', "true");
		$('.ui-toolbar-checkbox input:checkbox').each(function() {this.checked = false;});
	}
	initCheckElementList();
}

/**
*Inicializa el efecto :active de las listas en IE
*/
